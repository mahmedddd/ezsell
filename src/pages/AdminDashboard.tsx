...
export default function AdminDashboard() {
  ...
  const [currentChatInfo, setCurrentChatInfo] = useState<{user1: string, user2: string} | null>(null);
  ...
  const handleViewChat = async (user1Id: number, user2Id: number, user1Name: string, user2Name: string) => {
    try {
      setLoading(true);
      const messages = await adminService.getAdminConversation(user1Id, user2Id);
      setAdminChatMessages(messages);
      setCurrentChatInfo({ user1: user1Name, user2: user2Name });
      setIsAdminChatOpen(true);
    } catch (error) {
      toast({
        title: 'Error',
        description: 'Could not fetch conversation',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
                            ...
                            {report.status === 'pending' && (
                              <>
                                <Button
                                  ...
                                  onClick={() => handleViewChat(report.reporter_id, report.reported_id, report.reporter_username, report.reported_username)}
                                >
        ...
        <SheetContent className="w-full sm:max-w-md p-0 flex flex-col h-full overflow-hidden">
          <SheetHeader className="p-6 bg-[#2E6091] text-white">
            ...
            <SheetDescription className="text-white/70">
              Reviewing conversation between @{currentChatInfo?.user1} and @{currentChatInfo?.user2}
            </SheetDescription>
          </SheetHeader>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50 flex flex-col">
            {adminChatMessages.length === 0 ? (
              ...
              adminChatMessages.map((msg, index) => (
                <div key={index} className={`flex flex-col ${msg.sender_username === currentChatInfo?.user1 ? 'items-start' : 'items-end'}`}>
                  <div className="flex items-center gap-2 mb-1 px-1">
                    <span className="text-[10px] font-bold text-gray-400 uppercase tracking-tighter">@{msg.sender_username}</span>
                  </div>
                  <div className={`rounded-2xl px-4 py-2.5 max-w-[90%] text-sm shadow-sm transition-all hover:shadow-md ${
                    ...
                    {(() => {
                      try {
                        ...
                        if (parsed.type === 'offer') return `OFFER: Rs ${parsed.amount.toLocaleString()}`;
                        return msg.content;
                      } catch {
                        return msg.content;
                      }
                  ...
                  <span className="text-[9px] text-gray-400 mt-1 px-1">{new Date(msg.created_at).toLocaleString()}</span>
                </div>
              ))
            )}
          </div>
