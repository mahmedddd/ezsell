...
para(doc,
 'Rigorous testing was conducted across all ten modules to ensure the system is stable, '
 'secure, and performs accurately under real-world conditions. This included unit testing '
 'of isolated AI scripts, functional testing of UI flows for every module, and complex '
 'integration testing across third-party APIs.')

h2(doc,'6.1 Unit Testing')
para(doc, 'Unit testing validates that individual backend components, specifically the AI and ML logic scripts, perform as expected in isolation.')
para(doc, 'Table 10: Unit Testing Results', style='Caption')
simple_table(doc,
    ['No.', 'Test case / Component', 'Attribute and value', 'Expected result', 'Result'],
    [
        ...
        ['5', 'SentenceTransformer Embedding', 'Text: "Wooden dining table"', 'Outputs a 384-dimensional dense vector array.', 'Pass'],
        ['6', 'Scam Keyword Scanner', 'Text: "Message me on WhatsApp, advance payment only"', 'Boolean True (Scam detected).', 'Pass'],
    ]
)

h2(doc,'6.2 Functional Testing (Detailed by Module)')
para(doc, 'Functional testing ensures that every one of the 10 core modules meets the functional specifications detailed in the requirements phase.')
...
simple_table(doc,
    ...
    [
        ['1', 'M1: User Auth - Registration', 'Email: test@ezsell.com\nPhone: 03001234567', 'Account created, OTP sent to email, DB status Unverified.', 'OTP dispatched, status saved correctly.', 'Pass'],
        ['2', 'M2: Listings - Form Toggle', 'Select "Furniture" category', 'UI dynamically renders Material and Subtype fields.', 'Furniture fields rendered correctly.', 'Pass'],
        ['3', 'M3: AI Pricing - Prediction', 'Input: iPhone 13, 128GB, Excellent', 'Backend calculates ensemble price, returns PKR estimate.', 'Returns accurate PKR price with 88% confidence.', 'Pass'],
        ['4', 'M4: Recommendations - Feed', 'User History: Views 5 Laptops', 'Homepage prioritizes Laptop listings in the feed.', 'Feed correctly ranks laptops via cosine similarity.', 'Pass'],
        ['5', 'M5: AR Try-On - WebAR', 'Click "View in AR" button', 'model-viewer element loads GLB file, requests camera.', 'Camera overlay opens, GLB rendered to scale.', 'Pass'],
        ['6', 'M6: Admin - Moderation', 'Admin clicks "Reject" on listing', 'Status changes to Rejected, rejection reason saved.', 'Listing hidden from feed, status updated in DB.', 'Pass'],
        ['7', 'M7: Fraud - Content Hash', 'Submit listing with identical text/price', 'Fraud pipeline halts submission (Copy-paste error).', 'Submission blocked, UI shows duplicate error.', 'Pass'],
        ['8', 'M8: Messaging - Real-Time', 'Buyer sends message on listing', 'Message appears in seller inbox, unread badge increments.', 'Message stored, navbar badge updates to 1.', 'Pass'],
        ['9', 'M9: Favorites - Toggle', 'Click Heart icon on listing card', 'Listing added to /favorites, icon turns red.', 'Saved successfully, UI state reflects instantly.', 'Pass'],
        ['10', 'M10: Chatbot - Context API', 'Ask "What is a fair price for S22?"', 'Chatbot reads OLX CSV, answers with PKR price range.', 'Chatbot streams accurate PKR range via SSE.', 'Pass'],
    ]
)
...
simple_table(doc,
    ...
    [
        ['1', 'No', 'N/A', 'N/A', 'N/A', 'Hard Reject (403 Forbidden)', 'Pass'],
        ['2', 'Yes', 'Yes', 'N/A', 'N/A', 'Reject (Duplicate Listing)', 'Pass'],
        ['3', 'Yes', 'No', 'No', 'N/A', 'Reject (Category Mismatch on Image)', 'Pass'],
        ['4', 'Yes', 'No', 'Yes', 'Yes', 'Pending Review (Scam Flagged)', 'Pass'],
        ['5', 'Yes', 'No', 'Yes', 'No', 'Approved (Active Listing)', 'Pass'],
    ]
)
...
simple_table(doc,
    ...
    [
        ['1', 'LLM Pricing Pipeline (Groq)', 'Submit Mobile Specs to backend', 'Backend scrapes OLX, queries Groq API, returns PKR price.', 'Returned accurate PKR price in < 3s.', 'Pass'],
        ['2', 'Tripo 3D Generation (Tripo AI)', 'Upload furniture image to backend', 'Image to Cloudinary -> URL to Tripo -> Polling -> GLB save.', 'GLB URL returned and saved in DB.', 'Pass'],
        ['3', 'Chatbot SSE Stream (Groq)', 'Send message to /chat/stream', 'Frontend receives chunked Server-Sent Events from Groq.', 'Text streams dynamically in UI.', 'Pass'],
        ['4', 'Google OAuth 2.0 Flow', 'Click "Login with Google"', 'Google Auth window opens -> Redirects with Auth Code -> JWT.', 'JWT successfully issued to client.', 'Pass'],
    ]
)
...
doc.save(OUTPUT)
print(f'Chapters 6 (Detailed), 7, 8 done. Saved: {OUTPUT}')
